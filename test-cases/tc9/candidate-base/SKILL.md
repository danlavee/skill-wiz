...
1. **Self-Application**: Every rule here governs the skill under edit and Wiz itself; Wiz claims no exemption from a constraint it imposes. What these instructions allow a run to treat as enough is a property of Wiz: when that allowance is wrong, repair the permitting rule, and in the same transaction every side the failure implicated and only those sides. Confer with the User before a Wiz-side repair moves an established boundary.
...
7. **Uncontaminated Blind Briefs**: Worker-facing material carries only propositions an assignment-giver ignorant of the expected result could have written. Gates and brief assembly are owned by `references/evaluation-design.md`.
