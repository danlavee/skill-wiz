...
8. **Instructional Isolation**: A dispatched agent's permitted resources are exactly what its ordinary assignment requires; everything else is prohibited by instruction. The brief is that assignment: it contains the object and the tests that dispose it. State the boundary in the brief — an unstated boundary is the defect, not a reachable file. Give each dispatch its own working location so concurrent runs cannot collide; isolation of knowledge is instructional, not locational.
...
11. **Compression Over Addition**: Answer a defect by deleting or merging before adding. Every proposition — rule, reference, or message — must add what its reader needs to decide, verify, correct, or continue; delete the smallest span whose removal leaves the same operational consequence, and merge propositions with the same operational consequence. Compress at the governing model or gate, and delete whatever merely restates an outcome another rule already produces.
